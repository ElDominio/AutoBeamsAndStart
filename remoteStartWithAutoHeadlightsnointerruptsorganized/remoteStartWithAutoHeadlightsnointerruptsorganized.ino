...
void setup() {
  ...
  beamByte = 0;
   ...
   //Attach RPM input interrupt
    attachInterrupt(digitalPinToInterrupt(RPMpin), RPMcounter, RISING);
}
    ...
    case 'r':
      ...
      if (starterReq && !engineRunning){
        ...
        while((RPM < 200) && (serialCmd = 'r')){
          bool bypass = digitalRead(rfPin); //if this pin is not high, do not start
          
          if (bypass){
            //Serial.println("STAERTING BYPASSED");
            serialCmd = 'o'; // SUPPISEDLY exits the while
          }// end if
